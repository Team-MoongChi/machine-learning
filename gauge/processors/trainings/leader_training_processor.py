...
class LeaderTrainingProcessor(BaseTrainingProcessor):
  """
  리더 역할에 특화된 모델 훈련 클래스
  """
  ...
  def train_leader_model(self, model_params=None, cv_params=None):
    """
    리더 모델 훈련 파이프라인
  ...
  def get_leader_feature_importance(self):
    """리더 모델의 피처 중요도 반환"""
    importance_df = self.get_feature_importance(top_n=15)

    if importance_df is not None:
      print("=== 리더 모델 피처 중요도 (Top 15) ===")
